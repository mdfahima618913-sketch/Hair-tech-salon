...
const SEG_META: Record<Segment, { label: string; icon: any; cls: string }> = {
  all:      { label: 'All',      icon: Users,     cls: 'bg-white/8  border-white/10  text-gray-400'  },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          <div key={label} className="bg-zinc-900 border border-white/12 rounded-2xl p-3 text-center">
            <Icon size={16} className={`${color} mx-auto mb-1.5`}/>
            <p className={`font-black text-lg leading-none ${color}`}>{value}</p>
            <p className="text-[8px] text-gray-400 uppercase tracking-wider mt-1">{label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-zinc-900 border border-white/12 rounded-2xl overflow-hidden">
        ...
        <div className="p-5 space-y-2 max-h-72 overflow-y-auto scrollbar-hide">
          {(visitTab === 'active' ? activeList : inactiveList).slice(0,20).map(c => {
            ...
            return (
              <div key={c.phone} className="flex items-center gap-3 py-2 border-b border-white/10 last:border-0">
                <div className="w-8 h-8 rounded-full bg-white/8 flex items-center justify-center text-gray-400 font-black text-xs shrink-0">
      ...
      <div className="bg-zinc-900 border border-white/12 rounded-2xl overflow-hidden">
        ...
        <div className="max-h-96 overflow-y-auto scrollbar-hide">
          {filtered.slice(0, 50).map(c => {
            ...
            return (
              <div key={c.phone} className="border-b border-white/10 last:border-0">
                <button onClick={() => setOpenCust(isOpen ? null : c.phone)}
                  ...
                  <div className="w-9 h-9 rounded-full bg-white/8 flex items-center justify-center font-black text-gray-400 text-sm shrink-0">
                    {c.name?.charAt(0)?.toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                      ...
                      <div className="px-5 pb-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {[
                          ...
                          <div key={label} className="bg-white/[0.03] rounded-xl p-2.5">
                            <p className="text-[8px] uppercase tracking-wider text-gray-400 mb-0.5">{label}</p>
