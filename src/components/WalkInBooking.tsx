      ...
      {STEP_LABELS.map((label, i) => (
        <div key={label} className="flex items-center">
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-[10px] font-black uppercase tracking-wider hidden sm:inline ${i === idx ? 'text-gold' : i < idx ? 'text-emerald-400' : 'text-gray-600'}`}>{label}</span>
          </div>
          {i < STEP_LABELS.length - 1 && (
            <div className={`w-6 sm:w-10 h-px mx-1 ${i < idx ? 'bg-emerald-500/50' : 'bg-white/5'}`}/>
          )}
        </div>
      ))}
    </div>
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b border-white/8 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-600 text-[10px] mt-0.5">Creating on behalf of customer · Pay at Salon</p>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 'customer' && (
              <motion.div key="customer" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }} className="space-y-5">
                ...
                {/* Phone first */}
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-2">Mobile Number <span className="text-red-400">*</span></label>
                ...
                {/* Name */}
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-2">Full Name <span className="text-red-400">*</span></label>
                  {autoFilled && (
                  ...
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Customer full name" value={customerName}
                      ...
                      className={`w-full bg-white/5 border rounded-xl py-3 pl-9 pr-4 text-white text-sm focus:outline-none transition-all placeholder:text-gray-700 ${autoFilled ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-white/10 focus:border-gold/50'}`}
                    />
                  </div>
                </div>
              </motion.div>
                    ...
                    <div key={cat} className="border border-white/8 rounded-xl overflow-hidden">
                      <button onClick={() => setOpenCat(openCat === cat ? null : cat)}
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] text-gray-600">{(svcs as Service[]).length}</span>
                                ...
                                return (
                                  <div key={svc.id} className={`flex items-center justify-between px-4 py-2 ${q > 0 ? 'bg-gold/5' : 'hover:bg-white/[0.03]'} transition-all`}>
                                    <div className="min-w-0 flex-1">
                                      ...
                                      <p className="text-[9px] text-gray-600">{svc.time}</p>
                                    </div>
            ...
            {step === 'slot' && (
              <motion.div key="slot" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }} className="space-y-5">
                ...
                {/* Date strip */}
                <div>
                  <p className="text-[9px] font-black uppercase tracking-wider text-gray-600 mb-3">Choose Date</p>
                  <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                    {dateStrip.map(d => {
                      const active = isSameDay(d, selDate);
                      return (
                        <button key={d.toString()} onClick={() => setSelDate(d)}
                          className={`shrink-0 w-14 h-16 rounded-2xl flex flex-col items-center justify-center transition-all border-2 ${active ? 'bg-gold border-gold text-black' : 'bg-white/[0.03] border-white/8 text-gray-400 hover:border-gold/30'}`}>
                ...
                <div className="bg-white/[0.03] border border-teal-500/20 rounded-2xl overflow-hidden">
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    <div className="border-t border-white/5 pt-3">
                      ...
                      <div className="flex justify-between font-black text-sm pt-2 border-t border-white/5 mt-2">
            ...
            {step === 'success' && (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8 space-y-4">
                ...
                <div className="bg-white/[0.03] border border-white/8 rounded-2xl p-4 text-left space-y-2">
                  ...
                  {savedId && <div className="flex justify-between text-xs"><span className="text-gray-500">Ref ID</span><span className="text-gray-600 font-mono text-[10px]">{savedId.slice(-8)}</span></div>}
                </div>
                <button onClick={onClose} className="w-full py-3.5 bg-white/5 border border-white/10 rounded-2xl text-white font-bold text-sm hover:bg-white/10 transition-all">
                  Done
                </button>
              </motion.div>
            )}
